...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def _handle_validation(_request: Request, exc: RequestValidationError):
        return JSONResponse(
            status_code=422,
            content=error_body("invalid_input", "Request validation failed.", jsonable_encoder(exc.errors())),
        )

    @app.exception_handler(StarletteHTTPException)
    async def _handle_http(_request: Request, exc: StarletteHTTPException):
